...
// Host use only, will be ignored in (de)serialization
...
pub enum CudaCall {
    ...
    // we just add flash attention calls in an ad-hoc way
    FlashAttnCall {
    ...
    NcclCommSplit {
        ...
        color: i32,
        key: i32,
        // FOLLOWING FIELDS ARE HOST USE ONLY
        // IGNORE IN SIMULATOR
...
/// reports whether the event has completed at the host's current virtual time;
/// `end_time` is the event's completion time when ready (for
